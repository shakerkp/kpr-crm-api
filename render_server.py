...
def serve_app():
    """HTML app serve — file లేకపోయినా embedded version return చేస్తుంది."""
    from fastapi.responses import HTMLResponse
    # Try file first (local), then embedded
    for name in ["KPR_Lab.html", "KPR_Lab_CRM_Dashboard.html", "index.html"]:
        if os.path.exists(name):
            return FileResponse(name, media_type="text/html")
    # Embedded fallback — redirect to configure server
    return HTMLResponse(content="""<!DOCTYPE html>
<html><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/>
<title>KPR Lab CRM</title>
<style>body{font-family:system-ui,sans-serif;display:flex;align-items:center;justify-content:center;min-height:100vh;background:#fafaf9;margin:0}
.box{background:#fff;border-radius:16px;padding:32px;text-align:center;max-width:400px;border:1px solid #e7e5e4;box-shadow:0 8px 32px rgba(0,0,0,.07)}
h2{color:#1c1917;margin-bottom:8px}p{color:#78716c;font-size:14px}
.url{background:#f5f5f4;border-radius:8px;padding:8px 12px;font-family:monospace;font-size:13px;color:#ea580c;margin:12px 0}
</style></head><body>
<div class="box">
  <div style="font-size:48px;margin-bottom:16px">📷</div>
  <h2>KPR Colour Lab CRM</h2>
  <p>KPR_Lab.html file upload చేయండి లేదా below URL వాడండి:</p>
  <div class="url">""" + str(os.environ.get("RENDER_EXTERNAL_URL","https://kpr-crm-api.onrender.com")) + """/app</div>
  <p style="font-size:12px;color:#a8a29e">API is running ✅ — HTML file missing</p>
  <a href="/docs" style="color:#ea580c;font-size:13px">API Docs →</a>
