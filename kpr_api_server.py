...
try:
    ...
    NEON_SYNC = False
    print("[WARN] neon_sync.py not found — Neon real-time sync disabled")
import datetime
import hashlib
import os
import secrets
...
def serve_app():
    """PC2/Mobile లో HTML app serve చేయడానికి."""
    html_names = ["KPR_Lab.html", "KPR_Lab_CRM_Dashboard.html", "index.html"]
    for name in html_names:
        if os.path.exists(name):
            return FileResponse(name, media_type="text/html")
    return JSONResponse(
        status_code=404,
        content={"error": "HTML file కనుగొనబడలేదు. KPR_Lab.html ని server folder లో ఉంచండి."}
    )
